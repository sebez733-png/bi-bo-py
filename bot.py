...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE, custom_text=None):
    ...
    main_menu_buttons_am = [
        ...
        "🐝 Withdraw / ያውጡ", "📜 History / ታሪክ", "👤 Profile / መገለጫ",
        "🏢 Support / ድጋፍ", "🎁 Invite Friends / ጓደኛ ይጋብዙ", "🤖 Agent Panel",
        "🔄 Transfer / ይላኩ", "ℹ️ Info / መረጃ"
    ]
    main_menu_buttons_en = [
        "🎮 Open Game", "💳 Deposit", "💰 Balance", "🐝 Withdraw", "📜 History",
        "👤 Profile", "🏢 Support", "🎁 Invite Friends", "🤖 Agent Panel",
        "🔄 Transfer", "ℹ️ Info"
    ]
    ...
    if user_state.get(user_id) == "withdraw_method":
        ...
        withdraw_requests[req_num] = {"user_id": user_id, "amount": amount, "method": method, "phone": user_phone}
    ...
    if user_state.get(user_id) == "deposit_confirm":
        ...
        await update.message.reply_text(t('deposit_success', lang, method=method, amount=confirmed_amount, bonus=bonus, total=total, new_balance=new_balance), reply_markup=get_main_menu(lang))
        return

    if text in ["🔄 Transfer / ይላኩ", "🔄 Transfer"]:
        total_lifetime_deposits = get_total_deposits(user_id)
        if total_lifetime_deposits < 50:
            err_msg = "❌ ማዞር (መላክ) አይችሉም!\n\n⚠️ ገንዘብ ለማዞር (ለመላክ) 50 ብር ማስገባት አለብዎት።ፔ\n\n❌ You cannot transfer. You must deposit at least 50 ETB in total to unlock transfers." if lang == 'am' else "❌ Transfer locked!\n\n⚠️ You must deposit at least 50 ETB in total to unlock transfers."
            await update.message.reply_text(err_msg)
            return
    ...
    if user_state.get(user_id) == "transfer_select_wallet":
        ...
        phone_msg = "📱 Enter the registered phone number of the person you want to send to:\n\n(Example: 0912345678)" if lang == 'en' else "📱 ለመላክ የሚፈልጉትን ስልክ ቁጥር ያስገቡ:\n\n(ምሳሌ: 0912345678)"
    ...
    if user_state.get(user_id) == "transfer_amount":
        ...
        if amount < 10:
            ...
            await update.message.reply_text(err_msg)
            return
        if wallet_type == "Main Wallet": balance = get_main_balance(user_id)
        else: balance = get_play_balance(user_id)
        if amount > balance:
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_open_game":
        ...
        history = get_last_5_transactions(user_id)
        if not history: await query.message.reply_text("📜 ግብይት አልተደረገም / No transactions yet." if lang == 'am' else "📜 No transactions yet.")
        else:
            msg = "📜 LAST 5 TRANSACTIONS\n\n"
            for tx in history:
                tx_type, amount, time_str = tx; icon = "🟢 Deposit" if tx_type == "deposit" else "🔴 Withdraw"; clean_time = time_str.split('.')[0]
                msg += f"{icon}\n💰 Amount: {amount} ETB\n⏰ Date: {clean_time}\n\n"
            await query.message.reply_text(msg)
    elif data == "menu_profile":
        user = get_user(user_id)
        if user:
            link = f"https://t.me/{BOT_USERNAME}?start={user_id}"; ref_count = get_referral_count(user_id); played = get_games_played_count(user_id); won = get_games_won_count(user_id); total_won_amt = get_total_won(user_id)
            profile_msg = f"👤 PROFILE\n\n🆔 ID: {user[0]}\n📱 Phone: {user[1]}\n\n💰 Main Wallet: {user[2]} ETB\n🎮 Play Wallet: {user[3]} ETB\n\n🎯 Games Played: {played}\n🏆 Games Won: {won}\n💵 Total Won: {total_won_amt} ETB\n\n👥 Referrals: {ref_count}\n🎯 Invited By: {user[4] if len(user) > 4 and user[4] else 'No inviter'}\n\n🎁 Invite Link:\n{link}"
            await query.message.reply_text(profile_msg)
        else: await query.message.reply_text("❌ User not found")
    elif data == "menu_support":
        await query.message.reply_text("☎️ Support\n\nFor any comments or questions, contact support:\n@thelastking12312345678\n@Silencedoeir\n@one_day_82")
        ...
        total_lifetime_deposits = get_total_deposits(user_id)
        if total_lifetime_deposits < 50: await query.message.reply_text("❌ Transfer locked!\n\n⚠️ You must deposit at least 50 ETB in total to unlock transfers.")
        else:
            user_state[user_id] = "transfer_select_wallet"; keyboard = ReplyKeyboardMarkup([["Main Wallet", "Play Wallet"], ["🔙 Back"]], resize_keyboard=True)
            await query.message.reply_text("🔄 Select the wallet you want to send from:" if lang == 'en' else "🔄 ከየትኛው ዋሌት መላክ ይፈልጋሉ?", reply_markup=keyboard)
    elif data == "menu_info":
        await info(update, context, lang=lang)
...
async def approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    except: await update.message.reply_text("❌ Invalid number. Example: /ap 1"); return
...
async def reject(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id not in ADMIN_IDS: return
    if not context.args: await update.message.reply_text("❌ Please provide the request number. Example: /re 1"); return
    try: req_num = int(context.args[0])
    except: await update.message.reply_text("❌ Invalid number. Example: /re 1"); return
    if req_num not in withdraw_requests: await update.message.reply_text(f"❌ Request #{req_num} not found."); return
...
def on_connect():
    ...
    for room_id, game in game_states.items():
        ...
        emit('game_state_update', {
            'room': room_id, 'game_running': game['running'], 'game_id': game['game_id'],
            'time_left': time_left, 'total_players': count_total_cards(game),
            'called_numbers': list(game.get('called', [])), 'current_number': game.get('current'),
        })
...
def on_request_countdown(data):
    ...
    if not game['running']:
        ...
        socketio.emit('countdown_update', {'room': room, 'game_id': game['game_id'], 'time_left': 35}, room=f'bingo_room_{room}')

@socketio.on('player_ready')
def on_player_ready(data):
    room = data.get('room', '10')
    game = get_game_state(room)
    user_id = data.get('user_id'); name = data.get('name', 'Player'); cards = data.get('cards', []); game_id = data.get('game_id')

    if game_id == game.get('game_id') and not game.get('winner_declared', False):
        game['ready_players'][user_id] = {'name': name, 'cards': cards, 'card_num': cards[0] if cards else '—'}
        total = count_total_cards(game); game['total_players'] = total
    else:
...
def on_declare_winner(data):
    ...
    user_id = data.get('user_id'); winner_name = data.get('name', 'Player'); card_num = data.get('card_num', '—')
    card_index = data.get('card_index', 0); game_id = data.get('game_id', game.get('game_id'))

    if game.get('winner_declared', False): return
    
    game['winner_declared'] = True
    game['running'] = False  # ✅ STOP calling balls immediately!
    ...
    socketio.emit('winner_found', {
        'room': room, 'user_id': user_id, 'winner_name': winner_name, 'card_num': card_num,
        'card_index': card_index, 'prize': prize, 'total_players': total_players, 'game_id': game_id,
    }, room=f'bingo_room_{room}')

    # ✅ AUTO-RESET: Schedule room reset after 10 seconds (when winner screen finishes)
    def reset_room_after_winner():
        time_module.sleep(12)  # Give UI time to show winner
        game_states[room] = default_game_state()
        game_states[room]['timer_started_at'] = time_module.time()
        socketio.emit('countdown_update', {
            'room': room, 'game_id': game_states[room]['game_id'], 'time_left': 35
        }, room=f'bingo_room_{room}')

    threading.Thread(target=reset_room_after_winner, daemon=True).start()
...
def on_set_max_winners(data):
    room = data.get('room', '10'); game = get_game_state(room); mx = data.get('max', 1)
...
def api_win():
    ...
    if not user_exists(user_id): return jsonify({'success': False, 'error': 'User not found'}), 404
    user_data = db.get_user_full(user_id); status = user_data.get('status', 'active') if user_data else 'active'
    if status == 'banned': return jsonify({'success': False, 'error': 'Account banned'}), 403
    if status == 'frozen': return jsonify({'success': False, 'error': 'Account frozen'}), 403
...
def api_game_state():
    ...
    if not game['running']:
        if game['timer_started_at']:
            elapsed = int(now - game['timer_started_at']); time_left = max(0, 35 - elapsed)
        else:
            game['timer_started_at'] = now; time_left = 35
    return jsonify({
...
def api_start_game():
    ...
    game['running'] = True; game['game_id'] = data.get('game_id', ''); game['started_at'] = time_module.time()
    game['timer_started_at'] = None; game['total_players'] = 0; game['ready_players'] = {}; game['winner_declared'] = False
    return jsonify({'success': True})
...
def api_game_history():
    user_id = request.args.get('user_id', type=int)
    if not user_id or not user_exists(user_id): return jsonify({'success': False, 'error': 'User not found'}), 404
    rows = get_game_history(user_id, limit=20); history = []
    for row in rows: game_id, entry, status, result, ts = row; history.append({'game_id': game_id, 'entry': entry, 'status': status, 'result': result, 'time': ts})
...
def api_transactions():
    user_id = request.args.get('user_id', type=int)
    if not user_id or not user_exists(user_id): return jsonify({'success': False, 'error': 'User not found'}), 404
    rows = get_all_transactions(user_id, limit=20); txs = []
    for row in rows: tx_type, amount, status, ts = row; txs.append({'type': tx_type, 'amount': amount, 'status': status, 'time': ts})
...
def api_top_winners():
    ...
    winners = []
    for row in rows: uid, first_name, value = row; name = first_name if first_name and first_name.strip() else 'User'; winners.append({'name': name, 'value': value})
...
def api_admin_dashboard():
    if request.method == 'OPTIONS': return jsonify({'success': True}), 200
    try:
        stats = db.get_dashboard_stats(); stats['active_online'] = sum(count_total_cards(g) for g in game_states.values()); stats['running_games'] = sum(1 for g in game_states.values() if g.get('running')); stats['success'] = True
...
def api_approve_deposit():
    if request.method == 'OPTIONS': return jsonify({'success': True}), 200
    data = request.json or {}; deposit_id = data.get('deposit_id'); success, user_id, amount = db.approve_deposit(deposit_id)
    return jsonify({'success': success})

@flask_app.route('/api/admin/reject_deposit', methods=['POST', 'OPTIONS'])
def api_reject_deposit():
    if request.method == 'OPTIONS': return jsonify({'success': True}), 200
    data = request.json or {}; deposit_id = data.get('deposit_id'); success, user_id = db.reject_deposit(deposit_id)
    return jsonify({'success': success})
...
def api_approve_withdrawal():
    ...
    try:
        import asyncio
        async def send_approval(): await app.bot.send_message(chat_id=user_id, text=f"✅ Withdrawal Approved!\n\n💰 Amount: {amount} ETB\n🏦 The money has been sent to your account.")
        asyncio.run(send_approval())
    except Exception as e: print(f"Failed to send approval message to user {user_id}: {e}")
...
def api_reject_withdrawal():
    ...
    try:
        import asyncio
        async def send_rejection(): await app.bot.send_message(chat_id=user_id, text=f"❌ Withdrawal Rejected\n\n💰 Amount: {amount} ETB\n⚠️ Your request was rejected by admin. The money remains in your Main Wallet.")
        asyncio.run(send_rejection())
    except Exception as e: print(f"Failed to send rejection message to user {user_id}: {e}")
    if withdrawal_id in withdraw_requests: del withdraw_requests[withdrawal_id]
...
def api_add_balance():
    if request.method == 'OPTIONS': return jsonify({'success': True}), 200
    data = request.json or {}; user_id = data.get('user_id'); amount  = data.get('amount', 0); new_bal = db.update_main_balance(user_id, amount); db.add_transaction(user_id, 'admin_add', amount)
...
def api_remove_balance():
    if request.method == 'OPTIONS': return jsonify({'success': True}), 200
    data = request.json or {}; user_id = data.get('user_id'); amount  = data.get('amount', 0); new_bal = db.update_main_balance(user_id, -amount); db.add_transaction(user_id, 'admin_remove', amount)
    return jsonify({'success': True, 'main_balance': new_bal, 'play_balance': db.get_play_balance(user_id)})

@flask_app.route('/api/admin/add_main_balance', methods=['POST', 'OPTIONS'])
def api_add_main_balance():
    if request.method == 'OPTIONS': return jsonify({'success': True}), 200
    data = request.json or {}; user_id = data.get('user_id'); amount  = data.get('amount', 0); new_bal = db.update_main_balance(user_id, amount); db.add_transaction(user_id, 'admin_add_main', amount)
    return jsonify({'success': True, 'main_balance': new_bal, 'play_balance': db.get_play_balance(user_id)})

@flask_app.route('/api/admin/add_play_balance', methods=['POST', 'OPTIONS'])
def api_add_play_balance():
    if request.method == 'OPTIONS': return jsonify({'success': True}), 200
    data = request.json or {}; user_id = data.get('user_id'); amount  = data.get('amount', 0); new_bal = db.update_play_balance(user_id, amount); db.add_transaction(user_id, 'admin_add_play', amount)
...
def api_remove_main_balance():
    if request.method == 'OPTIONS': return jsonify({'success': True}), 200
    data = request.json or {}; user_id = data.get('user_id'); amount  = data.get('amount', 0); new_bal = db.update_main_balance(user_id, -amount); db.add_transaction(user_id, 'admin_remove_main', amount)
    return jsonify({'success': True, 'main_balance': new_bal, 'play_balance': db.get_play_balance(user_id)})

@flask_app.route('/api/admin/remove_play_balance', methods=['POST', 'OPTIONS'])
def api_remove_play_balance():
    if request.method == 'OPTIONS': return jsonify({'success': True}), 200
    data = request.json or {}; user_id = data.get('user_id'); amount  = data.get('amount', 0); new_bal = db.update_play_balance(user_id, -amount); db.add_transaction(user_id, 'admin_remove_play', amount)
    return jsonify({'success': True, 'main_balance': db.get_main_balance(user_id), 'play_balance': new_bal})

@flask_app.route('/api/admin/ban_user', methods=['POST', 'OPTIONS'])
def api_ban_user():
    if request.method == 'OPTIONS': return jsonify({'success': True}), 200
    data = request.json or {}; db.ban_user(data.get('user_id')); return jsonify({'success': True})

@flask_app.route('/api/admin/unban_user', methods=['POST', 'OPTIONS'])
def api_unban_user():
    if request.method == 'OPTIONS': return jsonify({'success': True}), 200
    data = request.json or {}; db.unban_user(data.get('user_id')); return jsonify({'success': True})

@flask_app.route('/api/admin/freeze_user', methods=['POST', 'OPTIONS'])
def api_freeze_user():
    if request.method == 'OPTIONS': return jsonify({'success': True}), 200
    data = request.json or {}; db.freeze_user(data.get('user_id')); return jsonify({'success': True})

@flask_app.route('/api/admin/unfreeze_user', methods=['POST', 'OPTIONS'])
def api_unfreeze_user():
    if request.method == 'OPTIONS': return jsonify({'success': True}), 200
    data = request.json or {}; db.unfreeze_user(data.get('user_id')); return jsonify({'success': True})
...
def api_set_max_winners():
    if request.method == 'OPTIONS': return jsonify({'success': True}), 200
    data = request.json or {}; room = data.get('room', '10'); game = get_game_state(room); mx = max(1, min(4, int(data.get('max_winners', 1)))); game['max_winners'] = mx
...
def api_pause_game():
    if request.method == 'OPTIONS': return jsonify({'success': True}), 200
    data = request.json or {}; room = data.get('room', '10'); game = get_game_state(room); game['paused'] = not game.get('paused', False)
    socketio.emit('game_paused', {'room': room, 'paused': game['paused']}, room=f'bingo_room_{room}')
...
def api_admin_rankings():
    ...
    rankings = []
    for r in rows: phone = get_user_phone(r[0]) or '—'; rankings.append({'user_id': r[0], 'name': r[1] or 'User', 'phone': phone, 'value': r[2]})
...
def api_admin_reports():
    ...
    try: return jsonify({'success': True, 'rows': db.get_admin_reports(period)})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 500
...
# ✅ FIXED: auto_call_loop now properly resets the called numbers for a new game, 
# and calls balls at 3 second intervals. Stops immediately if winner is declared.
def auto_call_loop():
    CALL_INTERVAL = 3  # Call a ball every 3 seconds
    while True:
        ...
        for room_id in list(game_states.keys()):
            ...
            if not game['running'] and game.get('timer_started_at') and not game.get('winner_declared'):
                ...
                if elapsed >= 35:
                    ...
                    game['winner_count'] = 0
                    game['called'] = []  # ✅ CRITICAL: Reset called numbers for new game!
                    game['current'] = None
